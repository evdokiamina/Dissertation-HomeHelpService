...
class mainAlgorithm:
    ...
    if status == GRB.Status.OPTIMAL:
      ...
      if not error:
        ...
        for i in selected:
          ...
          # plt.arrow( locations[i[0]][0],locations[i[0]][1],( locations[i[0]][0]-locations[i[1]][0]), (locations[i[0]][1]-locations[i[1]][1]), color='r')
        plt.show()
  ...
  def makeNoAPIAssignments(N, E, locations):
    ...
    if status == GRB.Status.OPTIMAL:
      ...
      if not error:
        for i in selected:
          # plt.arrow(locations[i[0]][0],locations[i[0]][1], locations[i[1]][0]-locations[i[0]][0], locations[i[1]][1]-locations[i[0]][1],width=0.00005,head_width=0.001)
          plt.plot((locations[i[0]][0],locations[i[1]][0]), (locations[i[0]][1],locations[i[1]][1]), color='r')
        plt.show()
        return(finalTour,e, n, 'First Try')
        exit(0)
