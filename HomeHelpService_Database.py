...
class useDatabase:
    def getDBData():
        ...
        try:
            ...
            #number of employees and clients can be changed to a fixed number
            e = random.randint(5, 10)
            n = random.randint(e,10)
            v = n + e
            N = [i for i in range(n)]
            E = [i for i in range(n, n+e)]  
            #location range can be changed to include further locations or restricted to closer ones
    ...
    def runAlgorithm( N, E, locations):
        ...
        if N != [] and E!=[] and locations!={}:
            ...
            # tour,e,n,relax  = HHS.makeAPIAssignments(N, E, locations) #use this method to make use of the Google Distance API - limit on uses depenting on budget - also needs configuration details and internet connenction
